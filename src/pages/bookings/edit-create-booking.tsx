...
export default function EditCreateBookings({ register, errors, control, action, slotData, setValue, query }: any) {
  ...
  const resourcesQuery = useOne({
    resource: "resources",
    id: slotData ? slotData?.slot.resourceId : query.data?.data.resourceId,
  });
  ...
  useEffect(() => {
    const addonIds = query.data?.data?.resourceAddons?.map((x: any) => x.resourceAddonId);
