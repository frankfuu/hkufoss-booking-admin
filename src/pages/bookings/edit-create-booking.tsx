...
export default function EditCreateBookings({
  register,
  errors,
  control,
  action,
  ...
  // addons chosen by user
  useEffect(() => {
    const addonIds = booking?.resourceAddons?.map((x: any) => x.resourceAddonId);
    setValue("resourceAddonRelations", addonIds);
  }, [booking]);

  // all possible addons
  const possibleAddonOptions = resourcesData?.data?.resourceAddons?.map((x: any) => ({
    label: x.resourceName,
    value: x.resourceAddonId,
  }));
            ...
            <Controller
              ...
              render={({ field }) => (
                <Autocomplete
                  ...
                  renderInput={(params) => (
                    <TextField {...params} label={t("resource")} margin="normal" variant="outlined" required />
                  )}
                />
              )}
            />
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Autocomplete
                      ...
                      getOptionLabel={(option) => `(ID: ${option?.id}) ${option?.name} (Acc. ${option?.accountCode})`}
                      renderInput={(params) => (
                        <TextField
                          {...params}
                          label={t("funders")}
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Autocomplete
                      multiple
                      {...field}
                      options={possibleAddonOptions || []}
                      getOptionLabel={(item) => {
                        if (!possibleAddonOptions) return "";
                        const option = possibleAddonOptions.find((p: any) => p.value == (item?.value ?? item));
                        return `${option?.label}`;
                      }}
                      onChange={(_, value) => {
                        const addonIds = value.map((item) => item?.value ?? item);
                        console.log(`onChange addOnIds`, addonIds);
                      ...
                      sx={{ mt: 2, mb: 1 }}
                      renderInput={(params) => (
                        <TextField {...params} label={t("resourceAddons")} InputLabelProps={{ shrink: true }} />
                      )}
                    />
                  )}
                />
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Autocomplete
                      ...
                      getOptionLabel={(option) => `(ID: ${option?.id}) ${option?.name} ${option?.code}`}
                      renderInput={(params) => (
                        <TextField
                          {...params}
                          label={t("courses")}
